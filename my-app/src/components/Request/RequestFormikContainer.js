...
const validationSchema = Yup.object().shape({
  ...
  email: Yup.string().email('Email должен содержать @').required('Поле не заполнено'),
    ...
    />
     <ErrorMessage name='remark' component={TextError}/>
                ...
                <div className="form_file">
                  <label for="file">
                  {values.file.name? <p>{values.file.name}</p>: <p>Прикрепите файл</p>}
...
<div className='checkbox_wrapper'>
          ...
          <ErrorMessage name='toggle' component={TextError} />
                  ...
                  <div>
                    <button  type="submit" onClick={props.goToRequestIsSent} disabled ={Object.keys(touched).length !== 0?!isValid :'true'} className = {Object.keys(touched).length === 0 || isValid===false?'button_dis' :'button'}>
                      Оставить заявку
                    </button>
                  </div>
                </div>
